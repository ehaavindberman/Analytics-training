# users visiting the site via organic spikes on day 8 and 
# then tapers off on day 9 and 10 back to normal. 
# This should change the overall signup rate because these users 
# are much less likely to sign up than others
...
# ===== Config ===== #
np.random.seed(12)
filename = '../../public/scenarios/scenario3.csv'
DAYS = 10
...
channel_weights = {"organic": 0.4, "paid": 0.3, "social": 0.2, "email": 0.1}
        ...
        for device, d_count in zip(device_keys, device_counts):
            ...
            for browser, b_count in zip(browser_keys, browser_counts):
                ...
                for channel, c_count in zip(channel_keys, channel_counts):
                    ...
                    if (day == two_days_ago.date()):
                        if channel == "organic":
                            c_count = c_count*5
                            conv_rate = conv_rate*.2
                        else:
                            c_count = round(c_count*.8)
                    elif (day > two_days_ago.date() and channel == "organic"):
                        c_count = c_count*3
                        conv_rate = conv_rate*.5
